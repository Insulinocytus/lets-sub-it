...
- 配置 LLM 相关环境变量。OpenAI 默认 endpoint 至少需要设置 `OPENAI_API_KEY`，或在启动命令中直接替换 `LSI_LLM_API_KEY` 的值；同时需要设置 `LSI_LLM_MODEL`。如使用其他 OpenAI-compatible 服务，还需要确认 `LSI_LLM_BASE_URL`。
...
先确保 `whisper-cli` 的虚拟环境已经通过上一步创建。从仓库根目录执行：
