...
**Tests**: Behavior changes MUST include module-appropriate deterministic tests.
Tests must stay offline and avoid real YouTube, model downloads, GPU, external
LLM calls, provider keys, private local data, or long-lived network dependencies.
Documentation-only changes may use `git diff --check` instead.
...
- [ ] T010 Confirm localhost-only backend origin, secret handling, and safe file-serving constraints remain intact
...
- [ ] TXXX Verify generated artifacts, databases, model files, caches, audio samples, `.env`, API keys, and LLM request logs are not committed
