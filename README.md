...
[概览](#概览) • [快速开始](#快速开始) • [真实模式](#真实下载转写与翻译模式) • [架构](#架构) • [API](#api) • [开发](#开发与验证)
...
> [!IMPORTANT]
> 项目仍处于 MVP 阶段。默认 backend 使用 mock runner，不访问 YouTube、Whisper 模型或 LLM。设置 `LSI_RUNNER_MODE=real` 后，backend 会调用 `yt-dlp`、`whisper-cli` 和 Chat Completions 兼容 LLM，生成真实 `source.vtt`、`translated.vtt` 和 `bilingual.vtt`。
...
| 模块 | 技术栈 | 当前能力 |
| --- | --- | --- |
| `backend/` | Go 1.22, SQLite, GORM | HTTP API、SQLite 持久化、job 复用、mock runner、可选真实下载、转写与翻译、VTT 文件服务 |
...
`LSI_RUNNER_MODE=real` 会让 backend 在 `downloading` 阶段调用 `yt-dlp`，在 `transcribing` 阶段调用 PATH 中的 `whisper-cli`，并在 `translating` 阶段调用 Chat Completions 兼容 LLM。
...
> [!NOTE]
> real runner 还要求本机安装 `yt-dlp` 和 `ffmpeg`，并配置可用的 Chat Completions 兼容 LLM。这个模式会生成真实 `audio.mp3`、`source.vtt`、`translated.vtt` 和 `bilingual.vtt`。
...
- real runner 的 LLM 翻译链路目前没有请求重试、并发控制或成本统计。
...
- [x] OpenAI-compatible LLM 翻译链路
- [x] 基于真实转写和翻译结果生成 `translated.vtt` / `bilingual.vtt`
- [ ] 更完整的任务历史、错误恢复和配置体验

## 排障

- `LSI_RUNNER_MODE=real` 启动失败：确认 `yt-dlp`、`ffmpeg` 和 `whisper-cli` 都能在当前 shell 的 `PATH` 中找到。
- real job 在 `translating` 阶段失败：确认 `LSI_LLM_MODEL`、`LSI_LLM_BASE_URL` 已配置；使用 OpenAI 默认 endpoint 时还需要配置 `LSI_LLM_API_KEY`。
