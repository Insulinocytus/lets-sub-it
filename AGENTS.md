...
- `backend/`：Go 1.22 API server。提供真实 HTTP API、SQLite 持久化、job 复用、默认 mock runner 状态推进、可选真实下载阶段和 VTT 字幕文件服务。
...
当前 backend 默认是“真实 API + 真实持久化 + mock runner”，不会访问 YouTube、调用 LLM 或执行真实转写。设置 `LSI_RUNNER_MODE=real` 时，`downloading` 阶段会调用本机 `yt-dlp` 并依赖 `ffmpeg`，但后续转写、翻译和打包仍使用 mock VTT。backend 调用 `whisper-cli`、OpenAI-compatible LLM 翻译，以及基于真实字幕生成 `translated.vtt` / `bilingual.vtt` 仍是后续迭代。
...
可选真实下载模式只替换 `downloading` 阶段，要求 `yt-dlp` 和 `ffmpeg` 都在 `PATH` 上：
...
- runner 位于 `backend/internal/runner/`；`MockRunner` 全阶段 mock，`RealRunner` 目前只有下载阶段真实调用 `yt-dlp`，不要把后续 mock VTT 描述成真实转写或真实翻译。
...
- backend 默认是 mock runner；`LSI_RUNNER_MODE=real` 只代表下载阶段真实执行，不代表端到端真实字幕链路。
