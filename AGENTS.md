...
- `backend/`：Go 1.22 API server。提供真实 HTTP API、SQLite 持久化、job 复用、默认 mock runner 状态推进、可选真实下载与转写阶段和 VTT 字幕文件服务。
...
当前 backend 默认是“真实 API + 真实持久化 + mock runner”，不会访问 YouTube、调用 LLM 或执行真实转写。设置 `LSI_RUNNER_MODE=real` 时，`downloading` 阶段会调用本机 `yt-dlp` 并依赖 `ffmpeg`，`transcribing` 阶段会调用本机 `whisper-cli` 生成真实 `source.vtt`；翻译和打包仍使用 mock VTT。OpenAI-compatible LLM 翻译，以及基于真实翻译生成 `translated.vtt` / `bilingual.vtt` 仍是后续迭代。
...
可选真实下载与转写模式要求 `yt-dlp`、`ffmpeg` 和 `whisper-cli` 都在 `PATH` 上：
...
- runner 位于 `backend/internal/runner/`；`MockRunner` 全阶段 mock，`RealRunner` 目前真实调用 `yt-dlp` 下载音频并调用 PATH 中本仓库的 Python `whisper-cli` 生成 `source.vtt`，不要把后续 mock `translated.vtt` / `bilingual.vtt` 描述成真实翻译结果。
